...
for (nclus in 1:4) {
  ...
  for(est in seq(dim(vazoe)[2])){
    ...
    modelo = Modelos[[est]]
    modelo$x = y
    graficos[[k_aux]] = autoplot(forecast(modelo,h = 24)) + 
      theme_minimal() + 
      ggtitle(label = "") + 
      xlab("Fecha") + 
    ...
    graficosX[[k_aux]] = autoplot(mod_pred) + 
      theme_minimal() + 
      ggtitle(label = "") + 
      xlab("Fecha") + 
      ylab(names(vazoe[est]))
...
#-------
save(graficos, nombres_cluster, file = "Predicciones/SARIMA.RData")
save(graficosX,nombres_cluster, file = "Predicciones/SARIMAX.RData")
