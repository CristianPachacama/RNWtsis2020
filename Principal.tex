...
Realizamos estudios comparativos entre ambos modelos, obteniendo resultados cercanos en cuanto a criterios de información y poder predictivo. A pesar de ello el modelo SARIMAX muestra una ventaja en todos los criterios, obteniendo, por ejemplo, para el caudal de la estación \textit{14 DE JULHO (284)},  un $AIC = 1913.2$, $\hat\sigma = 6655$ y el Log-Verosimilitud $= -950.6$, mejor en comparación con el modelo SARIMA, para el cual se obtuvieron un $AIC = 1882.2$, $\hat\sigma = 4884 $ y el Log-Verosimilitud $=-932.1$.
%Significado de Resultados .........................
Así mostramos que conseguimos pasar del problema de modelar 179 series de tiempo, a modelar las series de tiempo de unos cuantos clústers.
...
We carry out comparative studies between both models, obtaining close results in terms of information criteria and predictive power. Despite this, the SARIMAX model shows an advantage in all the criteria, obtaining, for example, for the flow of the station \textit{14 DE JULHO (284)}, an $AIC = 1913.2$, $\hat\sigma = 6655 $ and Log-Likelihood $ = -950.6 $, better in comparison with the SARIMA model, for which an $ AIC = 1882.2 $, $ \hat\sigma = 4884 $ and Log-Likelihood $= - 932.1$ were obtained.

% Meaning of Results .........................
Thus we show that we managed to move from the problem of modeling 179 time series, to modeling the time series of a few clusters.
...
Para ello haremos uso del Análisis de Componentes Principales (ACP) desarrollado por \citeauthor{pearson1901liii} \citeyear{pearson1901liii} y de manera independiente por \citeauthor{hotelling1933analysis} \citeyear{hotelling1933analysis}, y posteriormente estudiado a profundidad en  \cite{anderson2003multivariate,jolliffe2011principal}, qué como veremos más adelante es una técnica multivariante usada generalmente para reducir la dimensionalidad de los datos, y también como un complemento de los modelos lineales de regresión.
En particular usaremos una variante del ACP desarrollada específicamente para datos funcionales \cite{rao1958some,tucker1958determination}, el Análisis de Componentes Principales Funcional (ACPF) que es la técnica más adecuada para tratar con datos asociados a series de tiempo. 
  \citeauthor{ramsey1997functional}  \citeyear{ramsey1997functional} ofrecen una introducción al tema orientada a la aplicación, mientras que las propiedades asintóticas de consistencia de los estimadores están dispersas en varios artículos de diferentes autores en diferentes contextos: reducción de dimensión  \cite{dauxois1982asymptotic,hall2006properties,hall2009theory}, modelado lineal funcional \cite{cardot1999functional, cardot2003spline, hall2007methodology}, análisis de series de tiempo \cite{bosq2012linear}, por nombrar algunos. 
...
Finalmente, obtenidos un modelo SARIMA y SARIMAX por clúster, proponemos reciclar la información de estos modelos para usarla en el modelamiento de todas las series de caudales (179 en todo Brasil). %Recordemos que cada serie de tiempo está asociada a una estación de medición del caudal de un río. 
...
\begin{enumerate}
\item Una opción consiste de aplicar Análisis de Componentes principales Funcional (ACPF), a partir de este se obtiene un indicador temporal (serie de tiempo) que resume el comportamiento de todos los miembros del clúster. Hecho esto, se modela el indicador que representa el clúster mediante un modelo SARIMA. Así, se obtiene un modelo por cada clúster.
...
\paragraph{Nota.-} Toda esta metodología se automatizó, mediante la creación de una aplicación web, disponible al público, basada completamente en el lenguaje de programación R. Puede ver a detalle el desarrollo del código para esta investigación en el apéndice \ref{chp:webapp}. Todos los resultados mostrados a partir de este punto se pueden obtener en dicha aplicación.
...
Coef & Estimate & Std.Error & z-value & Pr(>|z|) & Signif\\
\midrule
\rowcolor{gray!6}  ar1 & 0.1767 & 0.0810 & 2.183 & 0.0291 & *\\
sar1 & 0.5594 & 0.1579 & 3.542 & 0.0004 & ***\\
\rowcolor{gray!6}  sar2 & 0.2902 & 0.0809 & 3.585 & 0.0003 & ***\\
sma1 & -0.7471 & 0.1592 & -4.692 & 0.0000 & ***\\
\rowcolor{gray!6}  intercept & 170.6881 & 10.6559 & 16.018 & 0.0000 & ***\\
\bottomrule
\end{tabular}
\begin{tablenotes}
\item \textit{Resumen:} 
\item sigma\textasciicircum{}2 = 7226.58, loglikelihood = -866.38, AIC = 1744.75, BIC =1762.73, Hannan-Quinn =1753.02
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\caption{\label{fig:sarma_precip} Residuos SARMA(1,0)(2,1) - Precipitación}\label{fig:unnamed-chunk-35}
\end{figure}
...
\caption{\label{fig:sarma_tmin} Residuos SARMA(2,2)(2,1) - Temperatura Mínima}\label{fig:unnamed-chunk-37}
\end{figure}
...
Ahora podemos pasar al paso 3 de la metodología \ref{sec:metod_sarimax}, que consiste de identificar los órdenes de los polinomios $\delta^{(k)}(B)$, $\omega^{(k)}(B)$ que componen las funciones de respuesta al impulso $v^{(k)}(B)$ asociadas a $x_t^{(k)}$. Para ello un primer paso consiste de estimar a groso modo los coeficientes $v_j^{(k)}$ de las funciones de respuesta al impulso, esto a partir de los valores de la función de correlación cruzada entre $\alpha_t^{(k)}$, hallada  en el paso anterior, y $\beta^{(k)}$ que resulta de aplicar el modelo SARMA de la $k$-ésima variable, pero a la serie $y_t$, es decir:
...
Ovservando la figura \ref{fig:acf_pertur} vemos que la función de autocorrelación y autocorrleación parcial de $n^{*}_t$ tienen picos fuera de las bandas de confianza en los retardos 1 y 10, en el resto de retardos muestra un comportamiento con picos periódicos, pero dentro de las bandas de confianza, por lo que podría ser necesario incluir polinomios estacionales en el modelo final. 
...
Coef & Estimate & Std.Error & z-value & Pr(>|z|) & Signif\\
\midrule
\rowcolor{gray!6}  ar1 & 0.536 & 0.069 & 7.772 & 0 & ***\\
intercept & 794.354 & 87.361 & 9.093 & 0 & ***\\
\bottomrule
\end{tabular}
\begin{tablenotes}
\item \textit{Resumen:} 
\item sigma\textasciicircum{}2 = 246947.21, loglikelihood = -1129.03, AIC = 2264.05, BIC =2273.04, Hannan-Quinn =2268.19
\end{tablenotes}
\end{threeparttable}
\end{table}
...
En el próximo capítulo se muestran los resultados de modelar todas las series que componen el clúster 1, partiendo de un modelo con los mismos parámetros (aunque no los mismos coeficientes).
...
\caption{\label{tab:unnamed-chunk-59}\label{tab:sarima1_resumen}Modelo SARIMA$(4,0,0)(1,1,0)_{12}$ - Clúster 1}
\centering
\resizebox{\linewidth}{!}{
...
\caption{\label{tab:unnamed-chunk-61}\label{tab:sarimax1_resumen}Modelo SARIMAX$(1,0,0)(2,1,0)_{12}$ - Clúster 1}
\centering
\resizebox{\linewidth}{!}{
...
\caption{\label{tab:unnamed-chunk-62}Matriz de Distancias}
\centering
\resizebox{\linewidth}{!}{
...
\caption{\label{tab:unnamed-chunk-63}Matriz de Distancias}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{lrrrrrrrrrrrrrrrrrrrr}
\toprule
...
\chapter{ACP Funcional}
\label{ap:acpf}

A continuación, resumimos en gráficos los resultados obtenidos de aplicar el Análisis de Componentes Principales Funcional a las series que componen los 4 clústers.
...
\begin{figure}[H]
\caption{ACPF del Series del Clúster 2}
\includegraphics[width=15cm]{2_Cluster/ACPF}
\label{fig:acp_summ2}
\centering
\end{figure}
...
\begin{figure}[H]
\caption{Superficie de Covarianza Estimada (Clúster 2)}
\includegraphics[width=15cm]{2_Cluster/covarianza}
\label{fig:covar2}
\centering
\end{figure}
...
\begin{figure}[H]
\caption{ACPF del Series del Clúster 3}
\includegraphics[width=15cm]{3_Cluster/ACPF}
\label{fig:acp_summ3}
\centering
\end{figure}

\begin{figure}[H]
\caption{Superficie de Covarianza Estimada (Clúster 3)}
\includegraphics[width=15cm]{3_Cluster/covarianza}
\label{fig:covar3}
\centering
\end{figure}
...
\begin{figure}[H]
\caption{ACPF del Series del Clúster 4}
\includegraphics[width=15cm]{4_Cluster/ACPF}
\label{fig:acp_summ4}
\centering
\end{figure}

\begin{figure}[H]
\caption{Superficie de Covarianza Estimada (Clúster 4)}
\includegraphics[width=15cm]{4_Cluster/covarianza}
\label{fig:covar4}
\centering
\end{figure}
...
\caption{\label{tab:unnamed-chunk-72}\label{tab:sarima_cl1}Modelo SARIMA$(4,0,0)(1,1,0)_{12}$}
\centering
\begin{threeparttable}
\begin{tabular}[t]{lrrrrl}
\toprule
...
\caption[Residuos - Test Portmanteau (Ljung-Box) SARIMA(4,0,0)(1,1,0)]{Residuos - Test Portmanteau (Ljung-Box) SARIMA(4,0,0)(1,1,0)}\label{fig:unnamed-chunk-73}
\end{figure}
...
\caption{\label{tab:unnamed-chunk-74}\label{tab:sarima_cl11}Modelo SARIMA$(4,0,0)(1,1,0)_{12}$ - Clúster 1}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{llllllrrrr}
\toprule
...
\caption{\label{tab:unnamed-chunk-76}\label{tab:sarima_cl2}Modelo SARIMA$(4,0,3)(1,1,0)_{12}$}
\centering
\begin{threeparttable}
\begin{tabular}[t]{lrrrrl}
\toprule
...
{\centering \includegraphics[width=\maxwidth]{figure/unnamed-chunk-77-1} 

}

\caption[Residuos - Test Portmanteau (Ljung-Box) SARIMA(4,0,3)(1,1,0)]{Residuos - Test Portmanteau (Ljung-Box) SARIMA(4,0,3)(1,1,0)}\label{fig:unnamed-chunk-77}
\end{figure}
...
\caption{\label{tab:unnamed-chunk-78}\label{tab:sarima_cl22}Modelo SARIMA$(4,0,3)(1,1,0)_{12}$ - Clúster 2}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{lllllllllrrrr}
\toprule
...
\caption{\label{tab:unnamed-chunk-80}\label{tab:sarima_cl3}Modelo SARIMA$(2,0,1)(1,1,0)_{12}$}
\centering
\begin{threeparttable}
\begin{tabular}[t]{lrrrrl}
\toprule
...
\caption[Residuos - Test Portmanteau (Ljung-Box) SARIMA(2,0,1)(1,1,0)]{Residuos - Test Portmanteau (Ljung-Box) SARIMA(2,0,1)(1,1,0)}\label{fig:unnamed-chunk-81}
\end{figure}
...
\caption{\label{tab:unnamed-chunk-82}\label{tab:sarima_cl33}Modelo SARIMA$(2,0,1)(1,1,0)_{12}$ - Clúster 3}
\centering
\resizebox{\linewidth}{!}{
...
\caption{\label{tab:unnamed-chunk-83}Modelo SARIMA$(2,0,1)(1,1,0)_{12}$ - Clúster 3}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{llllllrrrr}
\toprule
...
\caption{\label{tab:unnamed-chunk-84}Modelo SARIMA$(2,0,1)(1,1,0)_{12}$ - Clúster 3}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{llllllrrrr}
\toprule
...
\caption{\label{tab:unnamed-chunk-86}\label{tab:sarima_cl4}Modelo SARIMA$(3,0,2)(1,1,0)_{12}$}
\centering
\begin{threeparttable}
\begin{tabular}[t]{lrrrrl}
\toprule
...
{\centering \includegraphics[width=\maxwidth]{figure/unnamed-chunk-87-1} 

}

\caption[Residuos - Test Portmanteau (Ljung-Box) SARIMA(3,0,2)(1,1,0)]{Residuos - Test Portmanteau (Ljung-Box) SARIMA(3,0,2)(1,1,0)}\label{fig:unnamed-chunk-87}
\end{figure}
...
\caption{\label{tab:unnamed-chunk-88}\label{tab:sarima_cl44}Modelo SARIMA$(3,0,2)(1,1,0)_{12}$ - Clúster 4}
\centering
\resizebox{\linewidth}{!}{
...
\caption{\label{tab:unnamed-chunk-89}\label{tab:sarimax_cl1}Modelo SARIMAX$(1,0,0)(2,1,0)_{12}$}
\centering
\begin{threeparttable}
\begin{tabular}[t]{lrrrrl}
\toprule
...
{\centering \includegraphics[width=\maxwidth]{figure/unnamed-chunk-90-1} 

}

\caption[Residuos - Test Portmanteau (Ljung-Box) SARIMAX$(1,0,0)(2,1,0)_{12}$]{Residuos - Test Portmanteau (Ljung-Box) SARIMAX$(1,0,0)(2,1,0)_{12}$}\label{fig:unnamed-chunk-90}
\end{figure}
...
\caption{\label{tab:unnamed-chunk-92}\label{tab:sarimax_cl11}Modelo SARIMAX$(1,0,0)(2,1,0)_{12}$ - Clúster 1}
\centering
\resizebox{\linewidth}{!}{
...
\caption{\label{tab:unnamed-chunk-94}\label{tab:sarimax_cl2}Modelo SARIMAX$(1,0,0)(2,1,0)_{12}$}
\centering
\begin{threeparttable}
\begin{tabular}[t]{lrrrrl}
\toprule
...
\caption[Residuos - Test Portmanteau (Ljung-Box) SARIMAX$(1,0,0)(2,1,0)_{12}$]{Residuos - Test Portmanteau (Ljung-Box) SARIMAX$(1,0,0)(2,1,0)_{12}$}\label{fig:unnamed-chunk-95}
\end{figure}
...
\caption{\label{tab:unnamed-chunk-96}\label{tab:sarimax_cl22}Modelo SARIMAX$(1,0,0)(2,1,0)_{12}$ - Clúster 2}
\centering
\resizebox{\linewidth}{!}{
...
\caption{\label{tab:unnamed-chunk-98}\label{tab:sarimax_cl3}Modelo SARIMAX$(1,0,0)(1,1,0)_{12}$}
\centering
\begin{threeparttable}
\begin{tabular}[t]{lrrrrl}
\toprule
...
\caption[Residuos - Test Portmanteau (Ljung-Box) SARIMAX$(1,0,0)(1,1,0)_{12}$]{Residuos - Test Portmanteau (Ljung-Box) SARIMAX$(1,0,0)(1,1,0)_{12}$}\label{fig:unnamed-chunk-99}
\end{figure}
...
\caption{\label{tab:unnamed-chunk-100}\label{tab:sarimax_cl33}Modelo SARIMAX$(1,0,0)(1,1,0)_{12}$ - Clúster 3}
\centering
\resizebox{\linewidth}{!}{
...
\caption{\label{tab:unnamed-chunk-101}Modelo SARIMAX$(1,0,0)(1,1,0)_{12}$ - Clúster 3}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{lllllllrrrrl}
\toprule
...
\caption{\label{tab:unnamed-chunk-102}Modelo SARIMAX$(1,0,0)(1,1,0)_{12}$ - Clúster 3}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{lllllllrrrrl}
\toprule
...
\caption{\label{tab:unnamed-chunk-104}\label{tab:sarimax_cl4}Modelo SARIMAX$(0,0,3)(1,1,1)_{12}$}
\centering
\begin{threeparttable}
\begin{tabular}[t]{lrrrrl}
\toprule
...
\caption[Residuos - Test Portmanteau (Ljung-Box) SARIMAX$(0,0,3)(1,1,1)_{12}$]{Residuos - Test Portmanteau (Ljung-Box) SARIMAX$(0,0,3)(1,1,1)_{12}$}\label{fig:unnamed-chunk-105}
\end{figure}
...
\caption{\label{tab:unnamed-chunk-106}\label{tab:sarimax_cl44}Modelo SARIMAX$(0,0,3)(1,1,1)_{12}$ - Clúster 4}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{lllllllrrrrl}
\toprule
